(function(v,t,Z){function ka(){return(new Date).getTime()}function V(c){return S.call(c)==="[object Function]"}function $(c){return S.call(c)==="[object Array]"}function fa(c){return c&&S.call(c)==="[object Object]"}function pa(c){return(c||"").replace(/^\s+|\s+$/g,"")}function W(c,e){var f=false;if(fa(e)){c=c||{};for(var i in e){var j=e[i];if(c[i]!=j){f=f||{};f[i]=j}}}return f}function G(c){var e=[];if(c!=null){var f=c.length;if(f==null||typeof c==="string"||V(c)||c.setInterval)e[0]=c;else for(;f;)e[--f]=
...
f)===false)break}else if(r)for(i in c){if(e.call(c[i],i,c[i])===false)break}else for(f=c[0];j<l&&e.call(f,j,f)!==false;f=c[++j]);return c}function ga(c,e){for(var f=[],i,j=0,l=c.length;j<l;j++){i=e(c[j],j);if(i!=null)f[f.length]=i}return f.concat.apply([],f)}function qa(c){var e=[];if(typeof c=="object"){for(var f in c)e[e.length]=encodeURIComponent(f)+"="+encodeURIComponent(c[f]);return e.join("&").replace(A,"+")}return c}function H(c){c=w(true,c,w(true,{},la,c));var e,f,i=c.context||v,j=c.type.toUpperCase();
if(c.data&&c.processData&&typeof c.data!=="string")c.data=qa(c.data);if(c.cache===false&&j==="GET"){var l=ka(),r=c.url.replace(X,"$1_="+l+"$2");c.url=r+(r===c.url?(K.test(c.url)?"&":"?")+"_="+l:"")}if(c.data&&j==="GET")c.url+=(K.test(c.url)?"&":"?")+c.data;var s=false,o=c.xhr();c.username?o.open(j,c.url,c.async,c.username,c.password):o.open(j,c.url,c.async);try{c.data&&o.setRequestHeader("Content-Type",c.contentType);if(c.ifModified){ba[c.url]&&o.setRequestHeader("If-Modified-Since",ba[c.url]);ha[c.url]&&
...
!o.status&&location.protocol==="file:"||o.status>=200&&o.status<300||o.status===304||o.status===1223||o.status===0;break a}catch(Q){}y=false}if(y){if(y=c.ifModified){y=o;var E=c.url,J=y.getResponseHeader("Last-Modified"),ma=y.getResponseHeader("Etag");if(J)ba[E]=J;if(ma)ha[E]=ma;y=y.status===304||y.status===0}y=y?"notmodified":"success"}else y="error";y=y}e=y;if(e==="success")try{y=o;var ja=c.dataType;E=c;var a=y.getResponseHeader("content-type"),b=ja==="xml"||!ja&&a&&a.indexOf("xml")>=0,d=b?y.responseXML:
y.responseText;if(b&&d.documentElement.nodeName==="parsererror")throw"parsererror";if(E&&E.dataFilter)d=E.dataFilter(d,ja);if(typeof d==="string")if(ja==="json")d=typeof Y==="object"&&Y.parse?Y.parse(d):(new Function("return "+d))();f=d}catch(g){e="parsererror"}if(e==="success"||e==="notmodified")c.success&&c.success.call(i,f,e);else if(c.error)c.error.call(c.context||v,o,e,void 0);c.complete&&c.complete.call(i,o,e);aa&&o.abort();if(c.async)o=null}};if(c.async){var F=setInterval(L,13);c.timeout>0&&
...
c)},_ready:function(c){var e=this;e._unloadFun=v.onbeforeunload;v.onbeforeunload=function(){e._refresh()};e.trigger("ready",[c])},_go:function(){var c=this.data,e=this.history,f=this.buddy,i=this.room;e.option("userInfo",c.user);P(c.buddies,function(l,r){e.init("unicast",r.id,r.history)});f.handle(c.buddies);P(c.rooms,function(l,r){e.init("multicast",r.id,r.history)});f=this.setting.get("blocked_rooms");var j=c.rooms;$(f)&&j&&P(f,function(l,r){j[r]&&(j[r].blocked=true)});i.handle(j);i.options.ticket=
c.connection.ticket;this.trigger("go",[c]);this.connection.connect(c.connection);if((c=c.new_messages)&&c.length){P(c,function(l,r){r["new"]=true});this.trigger("message",[c])}},_stop:function(c,e){v.onbeforeunload=this._unloadFun;this.data.user.presence="offline";this.data.user.show="unavailable";this.buddy.clear();this.trigger("stop",[c,e])},autoOnline:function(){return!this.status.get("o")},_initEvents:function(){function c(j){var l={id:j.from,presence:j.type};if(j.show)l.show=j.show;if(j.nick)l.nick=
j.nick;if(j.status)l.status=j.status;return l}var e=this,f=e.history,i=e.buddy;e.connection.bind("connect",function(){}).bind("data",function(j){e.handle(j)}).bind("error",function(){e._stop("connect","Connect Error")}).bind("close",function(){e._stop("connect","Disconnect")});e.bind("message",function(j){for(var l=[],r=j.length,s=e.data.user.id,o,D,L,F=0;F<r;F++){o=j[F];L=o.type;D=L=="unicast"?o.to==s?o.from:o.to:o.to;o.id=D;if(L=="unicast"&&!o["new"]){D={id:D,presence:"online"};if(o.nick)D.nick=
o.nick;l.push(D)}}if(l.length){i.presence(l);i.complete()}f.handle(j)});e.bind("presence",function(j){i.presence(ga(j,c))})},handle:function(c){c.messages&&c.messages.length&&this.trigger("message",[c.messages]);c.presences&&c.presences.length&&this.trigger("presence",[c.presences]);c.statuses&&c.statuses.length&&this.trigger("status",[c.statuses])},sendMsg:function(c){c.ticket=this.data.connection.ticket;this.trigger("sendMsg",[c]);this.request({type:"post",url:this.options.urls.message,cache:false,
data:c})},sendStatus:function(c){c.ticket=this.data.connection.ticket;this.request({type:"post",url:this.options.urls.status,cache:false,data:c})},sendPresence:function(c){c.ticket=this.data.connection.ticket;this.data.user.show=c.show;this.status.set("s",c.show);this.request({type:"post",url:this.options.urls.presence,cache:false,data:c})},online:function(c){var e=this,f=e.status,i=[],j=[],l=f.get("tabs"),r=f.get("tabIds");r&&r.length&&l&&P(l,function(s){s[0]=="b"&&i.push(s.slice(2));s[0]=="r"&&
j.push(s.slice(2))});c=w({buddy_ids:i.join(","),room_ids:j.join(","),show:f.get("s")||"available"},c);e._ready(c);f.set("o",false);f.set("s",c.show);e.request({type:"post",dataType:"json",data:c,url:e.options.urls.online,success:function(s){if(s)if(s.success){s.user=w(e.data.user,s.user,{presence:"online"});e.data=s;e._go()}else e._stop("online",s.error_msg);else e._stop("online","Not Found")},error:function(){e._stop("online","Not Found")}})},offline:function(){var c=this.data;this.status.set("o",
...
presence:"webim/presence",refresh:"webim/refresh",status:"webim/status"}},log:ia,idsArray:da,now:ka,isFunction:V,isArray:$,isObject:fa,trim:pa,makeArray:G,extend:w,each:P,inArray:function(c,e){for(var f=0,i=e.length;f<i;f++)if(e[f]===c)return f;return-1},grep:function(c,e,f){for(var i=[],j=0,l=c.length;j<l;j++)!f!==!e(c[j],j)&&i.push(c[j]);return i},map:ga,JSON:Y,ajax:H,jsonp:z,comet:u,model:ea,objectExtend:x});ea("setting",{url:"/webim/setting",data:{blocked_rooms:[],play_sound:true,buddy_sticky:true,
minimize_layout:true,msg_auto_pop:true}},{_init:function(){this.request=this.options.jsonp?z:H;this.data=w({},this.options.data,this.data)},get:function(c){return this.data[c]},set:function(c,e){var f=this,i=c;if(c){if(typeof c=="string"){i={};i[c]=e}var j=f.data,l=W(j,i);if(l){P(l,function(r,s){f.trigger("update",[r,s])});i=w({},j,i);f.data=i;f.request({type:"post",url:f.options.url,dataType:"json",cache:false,data:{data:Y.encode(i)}})}}}});ea("status",{key:"_webim"},{_init:function(){var c=this.data;
if(c)this._save(c);else this.data=(c=T(this.options.key))?Y.decode(c):{}},set:function(c,e){var f=c;if(typeof c=="string"){f={};f[c]=e}var i=this.data;W(i,f)&&this._save(w({},i,f))},get:function(c){return this.data[c]},clear:function(){this._save({})},_save:function(c){this.data=c;T(this.options.key,Y.encode(c),{path:"/",domain:t.domain})}});ea("buddy",{url:"/webim/buddy"},{_init:function(){this.request=this.options.jsonp?z:H;this.data=this.data||[];this.dataHash={};this.handle(this.data)},clear:function(){this.data=
[];this.dataHash={}},count:function(c){var e=this.dataHash,f=0,i;for(var j in e)if(fa(c)){i=true;for(var l in c)if(c[l]!=e[j][l])i=false;i&&f++}else f++;return f},get:function(c){return this.dataHash[c]},complete:function(){var c=this.dataHash,e=[],f;for(var i in c){f=c[i];if(f.incomplete&&f.presence=="online"){f.incomplete=false;e.push(i)}}this.load(e)},update:function(c){this.load(c)},presence:function(c){var e=this.dataHash;c=$(c)?c:[c];for(var f in c){var i=c[f];i.presence=i.presence=="offline"?
"offline":"online";i.incomplete=!e[i.id]}this.handle(c)},load:function(c){c=da(c);c.length&&this.request({type:"get",url:this.options.url,async:true,cache:false,dataType:"json",data:{ids:c.join(",")},context:this,success:this.handle})},handle:function(c){var e=this.data,f=this.dataHash,i={};c=c||[];var j,l;for(var r in c){j=c[r];if(id=j.id){if(!f[id]){j.presence=j.presence||"online";j.show=j.show?j.show:j.presence=="offline"?"unavailable":"available";f[id]={};e.push(f[id])}j.incomplete=!!j.incomplete;
if(l=W(f[id],j)){j=l.presence||"update";i[j]=i[j]||[];w(f[id],l);i[j].push(f[id])}}}for(var s in i)this.trigger(s,[i[s]]);this.options.active&&this.complete()}});(function(){ea("room",{urls:{join:"/webim/join",leave:"/webim/leave",member:"/webim/members"}},{_init:function(){this.data=this.data||[];this.dataHash={};this.request=this.options.jsonp?z:H},get:function(c){return this.dataHash[c]},block:function(c){var e=this.dataHash[c];if(e&&!e.blocked){e.blocked=true;var f=[];P(this.dataHash,function(i,
j){j.blocked&&f.push(j.id)});this.trigger("block",[c,f])}},unblock:function(c){var e=this.dataHash[c];if(e&&e.blocked){e.blocked=false;var f=[];P(this.dataHash,function(i,j){j.blocked&&f.push(j.id)});this.trigger("unblock",[c,f])}},handle:function(c){var e=this,f=e.data,i=e.dataHash;P(c,function(j,l){var r=l.id;if(r){l.members=l.members||[];l.count=l.count||0;l.all_count=l.all_count||0;if(i[r])w(i[r],l);else{i[r]=l;f.push(l)}e.trigger("join",[i[r]])}})},addMember:function(c,e){var f=this;if($(e))P(e,
function(s,o){f.addMember(c,o)});else{var i=f.dataHash[c];if(i){for(var j=i.members,l,r=j.length;r--;)if(j[r].id==e.id)l=j[r];if(!l){e.nick=e.nick;j.push(e);i.count=j.length;f.trigger("addMember",[c,e])}}}},removeMember:function(c,e){var f=this.dataHash[c];if(f){for(var i=f.members,j,l=i.length;l--;)if(i[l].id==e){j=i[l];i.splice(l,1);f.count--}j&&self.trigger("removeMember",[c,j])}},initMember:function(c){var e=this.dataHash[c];if(e&&!e.initMember){e.initMember=true;this.loadMember(c)}},loadMember:function(c){var e=
this,f=e.options;e.request({type:"get",async:true,cache:false,url:f.urls.member,dataType:"json",data:{ticket:f.ticket,id:c},success:function(i){e.addMember(c,i)}})},join:function(c){var e=this,f=e.options;e.request({cache:false,type:"post",async:true,url:f.urls.join,dataType:"json",data:{ticket:f.ticket,id:c,nick:f.user.nick},success:function(i){e.initMember(c);e.handle([i])}})},leave:function(c){var e=this.options,f=this.dataHash[c],i=e.user;if(f){f.initMember=false;this.request({cache:false,type:"post",
url:e.urls.leave,data:{ticket:e.ticket,id:c,nick:i.nick}});this.trigger("leave",[f])}},clear:function(){}})})();ea("history",{urls:{load:"webim/history",clear:"webim/clear_history",download:"webim/download_history"}},{_init:function(){this.data=this.data||{};this.data.unicast=this.data.unicast||{};this.data.multicast=this.data.multicast||{};this.request=this.options.jsonp?z:H},unicast:function(c){return this.data.unicast[c]},multicast:function(c){return this.data.multicast[c]},handle:function(c){var e=
this.data,f={unicast:{},multicast:{}};c=G(c);var i=c.length,j,l,r=this.options.userInfo.id;if(i){for(var s=0;s<i;s++){j=c[s];o=j.type;if((l=o=="unicast"?j.to==r?j.from:j.to:j.to)&&o){f[o][l]=f[o][l]||[];f[o][l].push(j)}}for(var o in f)for(l in f[o]){j=f[o][l];if(e[o][l]){e[o][l]=e[o][l].concat(j);this._triggerMsg(o,l,j)}else this.load(o,l)}}},_triggerMsg:function(c,e,f){this.trigger(c,[e,f])},clear:function(c,e){var f=this.options;this.data[c][e]=[];this.trigger("clear",[c,e]);this.request({url:f.urls.clear,
type:"post",cache:false,data:{type:c,id:e}})},download:function(c,e){var f=this.options.urls.download;(new Date).getTime();var i=t.createElement("iframe"),j=new Date,l=[];j={id:e,type:c,time:(new Date).getTime(),date:j.getFullYear()+"-"+(j.getMonth()+1)+"-"+j.getDate()};for(var r in j)l[l.length]=encodeURIComponent(r)+"="+encodeURIComponent(j[r]);f+=(/\?/.test(f)?"&":"?")+l.join("&");i.setAttribute("src",f);i.style.display="none";t.body.appendChild(i)},init:function(c,e,f){if($(f)){this.data[c][e]=
f;this._triggerMsg(c,e,f)}},load:function(c,e){var f=this,i=f.options;f.data[c][e]=[];f.request({url:i.urls.load,async:true,cache:false,type:"get",dataType:"json",data:{type:c,id:e},success:function(j){f.init(c,e,j)}})}})})(window,document);
(function(v,t,Z){function ka(){return false}function V(a){var b="";if(a.length==0)return"";b=a.replace(/&/g,"&gt;");b=b.replace(/</g,"&lt;");b=b.replace(/>/g,"&gt;");b=b.replace(/    /g,"&nbsp;");b=b.replace(/\'/g,"&#39;");b=b.replace(/\"/g,"&quot;");return b=b.replace(/\n/g,"<br />")}function $(a){return/^http:\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"])*$/.test(a)}function fa(a){return a?a.replace(/<(?:.|\s)*?>/g,""):""}function pa(a,b){for(var d=[];a;a=a.nextSibling)a.nodeType==
...
b);d&&G(this,d)})}function qa(a,b,d){if(typeof d==="boolean")d?G(a,b):w(a,b);else W(a,b)?w(a,b):G(a,b)}function H(a){a&&a.style&&(a.style.display="block")}function z(a){a&&a.style&&(a.style.display="none")}function oa(a){a&&a.parentNode&&a.parentNode.removeChild(a)}function u(a,b,d){if(a.addEventListener)a.addEventListener(b,d,false);else{a["e"+b+d]=d;a[b+d]=function(){return a["e"+b+d](v.event)};a.attachEvent("on"+b,a[b+d])}}function T(a){if(a){a.preventDefault&&a.preventDefault();a.returnValue=
false}}function ia(a){if(!a.target)a.target=a.srcElement||t;if(a.target.nodeType===3)a.target=a.target.parentNode;return a.target}function R(a){a.setAttribute("unselectable","on");a.style.MozUserSelect="none";u(a,"selectstart",ka)}function da(){if(!s){s=true;if(o){for(var a,b=0;a=o[b++];)a();o=null}}}function ea(){if(!D){D=true;if(t.readyState==="complete")return da();if(t.addEventListener)t.addEventListener("DOMContentLoaded",function(){t.removeEventListener("DOMContentLoaded",arguments.callee,false);
...
ca(d)||(d={});a=d[a]===Z?a:d[a];if(b){I=b;for(var g in b)a=a.replace(/\{\{(.*?)\}\}/g,aa)}return a}function A(a,b){this.element=a;this.options=f({},A.defaults,b);this._init()}function na(a){a=a.getElementsByTagName("*");for(var b,d,g={},h=a.length-1;h>-1;h--){b=a[h];if((d=b.id)&&d.indexOf(":")==0)g[d.substring(1,d.length)]=b}return g}function K(a){var b=t.createElement("div");b.innerHTML=a;return b=b.firstChild}function X(a,b,d){function g(h,k){this.id=J++;this.name=a;this.className="webim-"+a;this.options=
f({},g.defaults,k);if(this.element=h||this.template&&K(this.template())||this.options.template&&K(Q(this.options.template))){this.options.className&&G(this.element,this.options.className);this.$=na(this.element)}M(this._init)&&this._init();M(this._initEvents)&&this._initEvents()}g.defaults=b;f(g.prototype,l,X.prototype,d);A[a]=g}function la(a,b){A.apps[a]=b||{}}function ba(a,b){return b?a=="b"||a=="buddy"||a=="unicast"?"b_"+b:"r_"+b:a}function ha(){t.selection&&(this.caretPos=t.selection.createRange())}
var ra=webim.idsArray,M=webim.isFunction,Y=webim.isArray,ca=webim.isObject,c=webim.trim,e=webim.makeArray,f=webim.extend,i=webim.each,j=webim.grep,l=webim.objectExtend,r=webim.map,s=false,o=[],D=false;S.timeSkew=0;S.init=function(a){S.timeSkew=(new Date).getTime()-parseFloat(a)};f(S.prototype,{getTime:function(){var a=this.date,b=a.getHours();a=a.getMinutes();if(a<10)a="0"+a;return b+":"+a+""},getDay:function(a){var b=this.date;if(a){a=new Date;a.setHours(0);a.setMinutes(0);a.setSeconds(0);a.setMilliseconds(0);
...
member:true,msgType:"multicast"},d),g);N||p.load("multicast",b);var U=m.chat(a,b);U.bind("sendMsg",function(C){n.sendMsg(C);p.handle(C)}).bind("downloadHistory",function(C){p.download("multicast",C.id)}).bind("select",function(C){q.presence(C);k.addChat("buddy",C.id,null,null,C.nick);m.focusChat("buddy",C.id)}).bind("block",function(C){B.block(C.id)}).bind("unblock",function(C){B.unblock(C.id)}).window.bind("close",function(){U.options.info.blocked&&B.leave(b)});setTimeout(function(){U.options.info.blocked?
B.join(b):B.initMember(b)},500);Y(h.members)&&i(h.members,function(C,sa){U.addMember(sa.id,sa.nick,sa.id==n.data.user.id)})}else{d=f({},N.buddyChatOptions,d);N=p.unicast(b);h=(O=q.get(b))||{id:b,nick:h||b};m.addChat(a,h,f({history:N,block:false,emot:true,clearHistory:true,member:false,msgType:"unicast"},d),g);O||q.update(b);N||p.load("unicast",b);m.chat(a,b).bind("sendMsg",function(C){n.sendMsg(C);p.handle(C)}).bind("sendStatus",function(C){n.sendStatus(C)}).bind("clearHistory",function(C){p.clear("unicast",
...
d=/\<\%\=(.*?)\%\>/ig;return function(g,h){if(!g)return"";b=h;return g.replace(d,a)}}(),E={add:function(a,b,d){a=A[a].prototype;for(var g in d){a.plugins[g]=a.plugins[g]||[];a.plugins[g].push([b,d[g]])}},call:function(a,b,d){if((b=a.plugins[b])&&a.element.parentNode)for(var g=0;g<b.length;g++)a.options[b[g][0]]&&b[g][1].apply(a.element,d)}},J=1;f(X.prototype,{_init:function(){}});f(A,{version:"3.0.1",widget:X,app:la,plugin:E,i18n:x,date:S,ready:function(a){ea();s?a():o.push(a)},createElement:K,defaults:{},
...
oa(d.tabTip);b.count&&this.notifyUser("information",b.count);ma(this)},notifyUser:function(a,b){var d=this.$;if(a=="information")if(this.isMinimize())if(y(d.tabCount,b)){G(d.tab,"ui-state-highlight");w(d.tab,"ui-state-default")}},_count:function(){return y(this.$.tabCount)},title:function(a,b){var d=this.$,g=d.tabIcon;if(b)if($(b)){g.className="webim-icon";g.style.backgroundImage="url("+b+")"}else g.className="webim-icon webim-icon-"+b;d.tabTipC.innerHTML=a;g=this.options.titleVisibleLength;if(a){for(var h=
...
g(h)});u(d,"mouseover",function(){G(this,"ui-state-hover");w(this,"ui-state-default")});u(d,"mouseout",function(){w(this,"ui-state-hover");this.className.indexOf("ui-state-")==-1&&G(this,"ui-state-default")});u(d,"mousedown",g);R(d);i(pa(b.actions.firstChild),function(h,k){ga(k,"ui-state-hover")});i(["minimize","maximize","close","tabClose"],function(h,k){u(b[k],"click",function(m){this.disabled||a[k]();g(m)});u(b[k],"mousedown",g)})},height:function(){return this.$.content.offsetHeight},_fitUI:function(){},
isMaximize:function(){return W(this.element,"webim-window-maximize")},isMinimize:function(){return W(this.element,"webim-window-minimize")}});var ma=function(){var a=false,b=function(){a&&a.deactivate();a=false;return true},d=function(){this&&this!=a&&b()&&(a=this)},g=function(){this&&a==this&&(a=false)};u(t,"mousedown",function(h){h=ia(h);for(var k;h;)if(h.id==":webim-window"){k=h;break}else h=h.parentNode;if(k)(h=k.window)&&h.activate();else b()});return function(h){if(h.active()){b();a=h}h.bind("activate",
...
n.push(h);n.push("</p>");return n.join("")},_renderDateBreak:function(a){var b=this._lastLogItem,d=new Date,g=new Date,h=[];d.setTime(a);b&&g.setTime(b.timestamp);if(!b||d.getDate()!=g.getDate()||d.getMonth()!=g.getMonth()){h.push("<h5>");h.push((new S(a)).getDay(true));h.push("</h5>")}return h.join("")},ui:function(a){return f({element:this.element,$:this.$},a)},plugins:{}});var ja=function(){function a(g,h){return'<a href="'+(h=="www."?"http://"+g:g)+'"'+d+">"+g+"</a>"}function b(g,h){d+=" "+g+
'="'+h+'"'}var d;return function(g,h){d="";h&&ca(h)&&i(h,b);return g.replace(/(https?:\/\/|www\.)([^\s<]+)/ig,a)}}();A.history.defaults.parseMsg=true;E.add("history","parseMsg",{render:function(a,b){var d=b.msg.body;d=V(d);d=ja(d,{target:"_blank"});b.msg.body=d}});A.history.defaults.emot=true;E.add("history","emot",{render:function(a,b){b.msg.body=A.emot.parse(b.msg.body)}});X("emot",{template:'<div class="webim-emot ui-widget-content"><%=emots%></div>'},{_init:function(){var a=this,b=a.element;i(b.firstChild.childNodes,
function(d,g){u(g,"click",function(){w(b,"webim-emot-show");a.trigger("select",this.firstChild.getAttribute("rel"))})})},template:function(){var a=this.emots=webim.ui.emot.emots,b=[];b.push('<ul class="ui-helper-clearfix">');i(a,function(d,g){var h=g.src,k=g.t?g.t:g.q[0];b.push('<li><img src="');b.push(h);b.push('" title="');b.push(k);b.push('" alt="');b.push(g.q[0]);b.push('" rel="');b.push(g.q[0]);b.push('" /></li>')});b.push("</ul>");return Q(this.options.template,{emots:b.join("")})},toggle:function(){qa(this.element,
...
h=na(g);d.member=h.ul;d.memberCount=h.memberCount;d.content.parentNode.insertBefore(g,d.content)}});A.chat.defaults.downloadHistory=true;E.add("chat","downloadHistory",{init:function(a,b){var d=b.self,g=K(Q('<a style="float: right;" href="#chat-downloadHistory" title="<%=download history%>"><em class="webim-icon webim-icon-download"></em></a>'));u(g,"click",function(h){T(h);d.trigger("downloadHistory",[d.options.info])});b.$.tools.appendChild(g)}});la("buddy",{init:function(a){a=a||{};var b=this,
...
function(q,B){q=="buddy_sticky"&&k.window.option("sticky",B)});k.bind("select",function(q){b.addChat("buddy",q.id);b.layout.focusChat("buddy",q.id)});k.window.bind("displayStateChange",function(q){if(q!="minimize"){g.option("active",true);d.status.set("b",1);g.complete()}else{d.status.set("b",0);g.option("active",false)}});var m=function(q){return ca(q)?q.id:q},n=function(q){return q.show!="invisible"&&q.presence=="online"},p=function(q){return q.show=="invisible"};g.bind("online",function(q){k.add(j(q,
n))});g.bind("offline",function(q){k.remove(r(q,m))});g.bind("update",function(q){k.add(j(q,n));k.update(j(q,n));k.remove(r(j(q,p),m))});k.offline()},ready:function(){var a=this.buddy;z(a.$.logo);a.online()},go:function(){var a=this.buddy;this.user&&!this.user._initElement&&a.window.subHeader(this.user.element);a.titleCount();a.hideError()},stop:function(a,b){var d=this.buddy;d.offline();if(a=="online"||a=="connect")d.showError(b)}});X("buddy",{template:'<div id="webim-buddy" class="webim-buddy">\t\t<div id=":search" class="webim-buddy-search ui-state-default ui-corner-all"><em class="ui-icon ui-icon-search"></em><input id=":searchInput" type="text" value="" /></div>\t\t\t<div class="webim-buddy-content" id=":content">\t\t\t\t<div id=":logo" class="webim-buddy-logo">&nbsp;</div>\t\t\t\t<div class="ui-state-error webim-login-error ui-corner-all" style="display: none;" id=":error"></div>\t\t\t\t<div id=":empty" class="webim-buddy-empty"><%=empty buddy%></div>\t\t\t\t\t<ul id=":ul"></ul>\t\t\t\t\t\t</div>\t\t\t\t\t\t\t</div>',
...
e(a);for(var b=0;b<a.length;b++)this._updateOne(a[b])},add:function(a,b){a=e(a);for(var d=0;d<a.length;d++)this._addOne(a[d],b);this.titleCount()},removeAll:function(){var a=[],b=this.li;for(var d in b)a.push(d);this.remove(a);this.titleCount()},remove:function(a){var b,d,g=this.li,h,k=this.li_group;a=ra(a);for(var m=0;m<a.length;m++){b=a[m];if(d=g[b]){this.size--;if(h=k[b]){h.count--;h.count==0&&z(h.el);h.title.innerHTML=h.name+"("+h.count+")"}oa(d);delete g[b]}}this.titleCount()},select:function(a){(a=
this.li[a])&&a.firstChild.click();return a},hideError:function(){z(this.$.error)},showError:function(a){var b=this.$.error;b.innerHTML=x(a);H(b)},destroy:function(){}});la("visitorstatus",{init:function(){var a=this.im,b=a.status,d=b.get("v_f"),g=b.get("v_l"),h=t.referrer,k=t.location.href,m=t.location.host;if(h&&h!=d){var n=(ex=/\/\/([^\/]+)/.exec(h))&&ex[1];if(n&&n!=m)b.set("v_f",h);else h=d}else h=d;k!=g&&b.set("v_l",k);this.visitorstatus=k+(h?" | "+x("from")+" "+h:"");var p={};a.bind("sendMsg",
function(q){var B="v_to_"+q.to;if(!p[B]){p[B]=true;var N="",O=a.status.get(B);if(!O||k!=g||h!=d){a.status.set(B,1);N=x("location")+": "+k;if(h&&(!O||h!=d))N+=" \n "+x("from")+": "+h}N&&a.sendMsg(f({},q,{body:N,"transient":true}))}})},ready:function(a){a.visitorstatus=this.visitorstatus}})})(window,document);
