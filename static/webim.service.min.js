...
j.show;if(j.nick)l.nick=j.nick;if(j.status)l.status=j.status;return l}var e=this,f=e.history,i=e.buddy;e.connection.bind("connect",function(){}).bind("data",function(j){e.handle(j)}).bind("error",function(){e._stop("connect","Connect Error")}).bind("close",function(){e._stop("connect","Disconnect")});e.bind("message",function(j){for(var l=[],r=j.length,s=e.data.user.id,o,D,L,F=0;F<r;F++){o=j[F];L=o.type;D=L=="unicast"?o.to==s?o.from:o.to:o.to;o.id=D;if(L=="unicast"&&!o["new"]){D={id:D,presence:"online"};
if(o.nick)D.nick=o.nick;l.push(D)}}if(l.length){i.presence(l);i.complete()}f.handle(j)});e.bind("presence",function(j){i.presence(ga(j,c))})},handle:function(c){c.messages&&c.messages.length&&this.trigger("message",[c.messages]);c.presences&&c.presences.length&&this.trigger("presence",[c.presences]);c.statuses&&c.statuses.length&&this.trigger("status",[c.statuses])},sendMsg:function(c){c.ticket=this.data.connection.ticket;this.trigger("sendMsg",[c]);this.request({type:"post",url:this.options.urls.message,
cache:false,data:c})},sendStatus:function(c){c.ticket=this.data.connection.ticket;this.request({type:"post",url:this.options.urls.status,cache:false,data:c})},sendPresence:function(c){c.ticket=this.data.connection.ticket;this.data.user.show=c.show;this.status.set("s",c.show);this.request({type:"post",url:this.options.urls.presence,cache:false,data:c})},online:function(c){var e=this,f=e.status,i=[],j=[],l=f.get("tabs"),r=f.get("tabIds");r&&r.length&&l&&P(l,function(s){s[0]=="b"&&i.push(s.slice(2));
s[0]=="r"&&j.push(s.slice(2))});c=w({buddy_ids:i.join(","),room_ids:j.join(","),show:f.get("s")||"available"},c);e._ready(c);f.set("o",false);f.set("s",c.show);e.request({type:"post",dataType:"json",data:c,url:e.options.urls.online,success:function(s){if(s)if(s.success){s.user=w(e.data.user,s.user,{presence:"online"});e.data=s;e._go()}else e._stop("online",s.error_msg);else e._stop("online","Not Found")},error:function(){e._stop("online","Not Found")}})},offline:function(){var c=this.data;this.status.set("o",
